        ...
        <form action="{{ route('admin.companies.update', ['company' => $companyInfo->id]) }}" method="post" class="w-full divide divide-y">
            ...
            <div class="p-4">
                <div class="flex items-center">
                    <span class="w-1/4 pl-1 font-bold">掲載状況</span>
                    <span class="flex-auto flex items-center gap-8">
                        <label for="publish"><input type="radio" id="publish" name="is_publish" value="1" class="mr-2" @if($companyInfo->is_publish === 1){ checked } @endif>掲載中</label>
                        <label for="not_publish"><input type="radio" id="not_publish" name="is_publish" value="0" class="mr-2" @if($companyInfo->is_publish === 0){ checked } @endif>非掲載</label>
                        @error('is_publish')
                        <p class="text-rose-700 mt-2">{{ $message }}</p>
                        @enderror
                    </span>
